...
function expectation(
  ...
  return {
    ...
    external_session_id: `example-session-${number}`,
...
test("adapter renders four sorted Current items and keeps Withdrawn history separate", () => {
      ...
      changes: [
        {
          ...
          expectation_key: "local_file:example-event:example-session-5",
          title: "Program 5",
          external_session_id: "example-session-5",
          fields: [],
        },
      ],
  ...
  assert.deepEqual(workspace.automation, {
    ...
    programLastFailureAt: undefined,
  });
});
