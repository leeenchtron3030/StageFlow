...
class VisionObservationInterpreter:
    """Concrete interpreter for objective vision observations."""

    id: EntityId
    name: str = "Vision observation interpreter"
    ...
    mappings: Sequence[VisionObservationMapping] = field(
        default_factory=lambda: VISION_OBSERVATION_MAPPINGS
    )
    rules: Sequence[VisionInterpreterRule] = field(default_factory=tuple)
    ...
    def supported_event_sources(self) -> tuple[ProductionEventSource, ...]:
        return (ProductionEventSource.VISION_SYSTEM,)

    @property
    def intended_observation_types(self) -> tuple[ObservationType, ...]:
        return (ObservationType.VISION_ACTIVITY,)
    ...
    def _event_tuple(
        ...
        if not event_tuple:
            raise ValueError(
                "VisionObservationInterpreter requires at least one ProductionEvent."
            )
        return event_tuple
    ...
    def _observation_for_event(
        ...
        rule_id = next(
            ...
            f"vision:{event.event_type.value}:{mapping.vision_lifecycle}",
        )
        return (
            Observation(
                id=EntityId.new(),
                ...
                observation_source=ObservationSource.VISION,
                ...
                metadata={
                    "vision_is_not_meaning": True,
                    "vision_lifecycle": mapping.vision_lifecycle,
                    "visual_detection_id": event.payload.get("visual_detection_id"),
                    "visual_detection_type": event.payload.get("visual_detection_type"),
                    "visual_detection_status": event.payload.get("visual_detection_status"),
                    "timeline_range_reference": event.payload.get(
                        "timeline_range_reference"
                    ),
                    "confidence": event.payload.get("confidence"),
                    "region_reference": event.payload.get("region_reference"),
                },
                notes=mapping.observation_note,
                provenance=observation_provenance_from_event(
                    event,
                    interpreter_id=self.id,
                    interpreter_kind="vision_activity_interpreter",
    ...
    def _mapping_for_event(
        ...
        mapping = mapping_for_vision(
            event.event_type,
            visual_detection_type=event.payload.get("visual_detection_type"),
        )
        if mapping is not None and mapping.requires_vision_metadata:
            if event.metadata.get("vision_adapter_event") is not True:
                return None
        return mapping
