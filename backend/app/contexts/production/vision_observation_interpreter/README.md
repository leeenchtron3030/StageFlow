...
Each produced Observation carries the exact vision Production Event ID, type, and
occurrence time; stable `vision_activity_interpreter` identity; applied mapping-rule
identity; and known stage, recording-block, correlation, and timeline context. No visual
content is inspected to invent missing context.
