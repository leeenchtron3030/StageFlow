...
class AutonomousEventNodeConfiguration(BaseModel):
    ...
    enabled: bool = False
    rehearsal_fault_media_cycle: int | None = Field(default=None, gt=0, strict=True)
...
class KernelDeploymentConfiguration(BaseModel):
    ...
    def supported_schema_and_network(self) -> KernelDeploymentConfiguration:
        ...
        if (
            self.autonomous_event_node.rehearsal_fault_media_cycle is not None
            and self.event_mode is not EventModePolicy.REHEARSAL
        ):
            raise ValueError("rehearsal_fault_media_cycle requires rehearsal event mode")
