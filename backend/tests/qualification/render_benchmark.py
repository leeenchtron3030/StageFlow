...
def _load_baseline(path: Path, probe: CorpusProbe) -> Mapping[str, object]:
    if not path.is_absolute():
...
def build_parser() -> argparse.ArgumentParser:
    ...
    concurrent.add_argument("--transcription-model", required=True)
    concurrent.add_argument("--transcription-model-version", required=True)
    concurrent.add_argument(
        "--cuda-library-directory", type=Path, required=True
    )
    concurrent.add_argument(
        "--transcription-device", default="cuda", choices=("cuda",)
    )
    concurrent.add_argument("--transcription-compute-type", default="float16")
    concurrent.add_argument("--transcription-language")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "concurrent":
            report = run_concurrent_arm(
                blocks,
                output_video,
                baseline_report_path=args.baseline_nvenc_report,
