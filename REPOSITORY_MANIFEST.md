...
ED-0049 adds the supplied-fact evaluation boundary between candidate discovery and the
ED-0048 declarations. Immutable candidate/resource and narrow snapshot, finalization,
write, read, and presence contracts feed exactly one conservative deterministic policy.
Strong and guarded stability routes retain explicit parameters, categorical outcomes,
reason/observation lineage, identity conflicts, limitations, and distinct timestamps
without collecting facts, reading media, implementing Runtime services, or constructing
and transferring a full asset.

ED-0050 adds one deployment-neutral immutable Runtime description for Agent, Node,
external-compatible, development, and unknown deployments. It declares identity,
version, host, configuration, capabilities, collection plans, ED-0049 readiness route
selection, production-subordinate resource and event behavior, ED-0048 asset assembly
mapping, limitations, health, availability, summaries, and deterministic combination
validation without adding a service, monitoring, collection, policy execution, asset
assembly, persistence, transfer, downstream state, API, worker, AI, or frontend behavior.

ED-0051 adds the first executable Runtime profile as exactly one synchronous,
process-local Software Agent lifecycle. It validates ED-0050 configuration on explicit
startup, records immutable state and transition lineage, maps supplied production
pressure to execution permission, requires explicit resume after suspension, and
provides deterministic cancellation, stop, idempotency, concurrency, and notification
failure semantics without media, filesystem, recorder, readiness, asset, transfer,
queue, persistence, network, service, downstream state, API, worker, AI, or frontend
behavior.

ED-0052 adds the first media-facing Agent execution boundary as one explicit,
synchronous, bounded coordinator over injected discovery and ED-0049 observation
ports. ED-0050 configuration and ED-0051 permission gate deterministic calls;
candidates deduplicate process-locally, conflicts remain first-class, and objective
facts accumulate atomically without filesystem/recorder adapters, readiness execution,
asset assembly, transfer, queues, persistence, networking, services, or downstream
semantic/state behavior.

ED-0053 adds the first concrete media-source adapter as one stateless implementation of
the ED-0052 discovery port. One explicit request performs bounded read-only metadata
inspection of one configured single file or shallow local/mounted directory, with
deterministic eligibility, ordering, identity, typed limitations, and request-anchored
time. It adds no watcher, polling, recursion, content access, readiness, asset assembly,
transfer, queue, persistence, network, service, or downstream semantic/state behavior.
...
| `backend/app/contexts/production/` | Production context package. | ED-0002 / ED-0005 / ED-0012 through ED-0040 / ED-0042 through ED-0053 | Contains backend-only Production contracts and behavior through acceptance, repository proof, completed-media/readiness contracts, declarative Runtime, Agent lifecycle, bounded collection, and one shallow read-only local-filesystem discovery adapter; no production persistence, ingest, monitoring, or operating-system service behavior. |
...
| `backend/app/contexts/production/completed_media_asset/completed_media_asset_source.py` | Completed media source contract. | ED-0048 / ED-0053 correction | Retains peer Agent, Node, external-compatible, Development, or genuinely unknown Runtime provenance first-class plus optional host, recorder, adapter, producer, Event, version, and compatibility references. |
...
| `backend/app/contexts/production/asset_readiness/` | Asset Stability and Readiness package. | ED-0049 | Immutable candidate/resource facts, observation bundle, explicit parameters, stability derivation, categorical evaluation, and exactly one conservative policy over supplied facts without collection, file access, Runtime services, asset assembly, transfer, or downstream state. |
| `backend/app/contexts/production/asset_readiness/README.md` | Asset readiness package guide. | ED-0049 | Documents mission priority, detection boundary, route semantics, precedence, limitations, deployment neutrality, ED-0048 output mapping, and excluded operational behavior. |
| `backend/app/contexts/production/asset_readiness/__init__.py` | Asset readiness package exports. | ED-0049 | Exports candidate, observation, parameter, window, policy, outcome, reason, evaluation, summary, and status contracts. |
| `backend/app/contexts/production/asset_readiness/media_asset_candidate.py` | Media asset candidate contract. | ED-0049 / ED-0053 correction | Retains proposed identity, first-class Agent/Node/external/Development/unknown Runtime provenance, explicit context, intended kind, relationship, and first-observed time without completion, readiness, workflow, or Session claims. |
| `backend/app/contexts/production/asset_readiness/media_asset_candidate_resource.py` | Candidate resource contract. | ED-0049 | Retains descriptive filename, opaque ED-0048 location, host/volume, and media hints without final size, checksum, modification time, duration, or readiness. |
| `backend/app/contexts/production/asset_readiness/asset_resource_snapshot.py` | Resource snapshot contract. | ED-0049 | Supplied point-in-time size, descriptive modification time, opaque identity token, host/volume, provenance, limitations, and metadata without resource access. |
| `backend/app/contexts/production/asset_readiness/asset_finalization_observation.py` | Finalization observation contract. | ED-0049 | Supplied ED-0048 completion-method signal with declaring identity, marker/Event references, limitations, and no finalization behavior. |
| `backend/app/contexts/production/asset_readiness/asset_write_state_observation.py` | Write-state observation contract. | ED-0049 | Supplied active, inactive, unknown, or unsupported write assessment without handle inspection. |
| `backend/app/contexts/production/asset_readiness/asset_read_access_observation.py` | Read-access observation contract. | ED-0049 | Supplied readable, unreadable, unknown, or unsupported non-destructive assessment and access scope without opening or reading media. |
| `backend/app/contexts/production/asset_readiness/asset_resource_presence_observation.py` | Resource-presence observation contract. | ED-0049 | Supplied present, missing, replaced, or unknown state with explicit replacement and opaque identity semantics. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_observation_bundle.py` | Asset readiness observation bundle. | ED-0049 | Deterministically orders and deduplicates immutable candidate/resource facts while retaining conflicting duplicate identities for policy classification. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_policy.py` | Abstract asset readiness policy. | ED-0049 | Pure evaluation interface over supplied candidate, bundle, and request without collection, action, asset construction, or infrastructure. |
| `backend/app/contexts/production/asset_readiness/conservative_asset_readiness_policy.py` | Conservative asset readiness policy. | ED-0049 | Sole concrete deterministic policy implementing precedence, strong/stable/manual routes, latest-state semantics, blockers, conflicts, and ED-0048 declaration mapping. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_policy_parameters.py` | Asset readiness policy parameters. | ED-0049 | Explicit immutable interval, capability requirements, accepted strong methods, and version with no deployment-derived thresholds. |
| `backend/app/contexts/production/asset_readiness/asset_stability_window.py` | Asset stability window derivation. | ED-0049 | Deterministically derives a qualifying compatible snapshot interval while retaining timing, identity, snapshot lineage, and limitations; it does not establish readiness alone. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_evaluation_request.py` | Asset readiness evaluation request. | ED-0049 | Supplies explicit IDs, policy version, evaluation time, and conditional ED-0048 declaration identities without implicit clock access. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_outcome.py` | Asset readiness outcome vocabulary. | ED-0049 | Categorical safe, not-safe, insufficient, conflicting, unsupported, invalid, and unknown outcomes without scoring. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_reason.py` | Asset readiness reason vocabulary. | ED-0049 | Deterministic immutable supporting, blocking, insufficient, conflict, request, and capability reason codes with ID-only observation lineage. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_evaluation.py` | Asset readiness evaluation contract. | ED-0049 | Immutable categorical result with explicit parameter lineage, reasons, IDs, optional window, limitations, and safe-only compatible completion/readiness output. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_summary.py` | Asset readiness summary. | ED-0049 | Privacy-safe projection of outcome, method, interval, reason categories, time, Runtime/profile provenance, limitation count, and warnings without source paths or metadata. |
| `backend/app/contexts/production/asset_readiness/asset_readiness_validation.py` | Asset readiness validation helpers. | ED-0049 | Reuses ED-0048 timezone, ID/string normalization, deep metadata freezing, and credential rejection for compatible deterministic contracts. |
| `backend/app/contexts/production/runtime/` | StageFlow Runtime contract package. | ED-0050 | One immutable deployment-neutral Runtime graph and deterministic combination validation without any running service or execution behavior. |
| `backend/app/contexts/production/runtime/README.md` | Runtime package guide. | ED-0050 | Documents identity, peer profiles, capabilities, production priority, offline event mode, ED-0049/ED-0048 mappings, health/availability separation, validation, and explicit non-goals. |
| `backend/app/contexts/production/runtime/__init__.py` | Runtime package exports. | ED-0050 | Exports all approved Runtime contracts, categorical vocabularies, summary, and pure validator. |
| `backend/app/contexts/production/runtime/stageflow_runtime.py` | Canonical StageFlow Runtime graph. | ED-0050 | Composes one immutable normalized Runtime description with no singleton, process, or lifecycle behavior. |
| `backend/app/contexts/production/runtime/runtime_profile.py` | Runtime profile vocabulary. | ED-0050 | Defines Agent, Node, external-compatible, development, and unknown descriptive peer profiles without trust ranking. |
| `backend/app/contexts/production/runtime/runtime_identity.py` | Runtime identity contract. | ED-0050 | Retains first-class Runtime, host, installation, organization, deployment, and Stage identity without Session identity. |
| `backend/app/contexts/production/runtime/runtime_version.py` | Runtime version contract. | ED-0050 | Declares semantic, compatibility, configuration, capability, build, and caller-supplied build-time facts. |
| `backend/app/contexts/production/runtime/runtime_host.py` | Runtime host description. | ED-0050 | Retains supplied platform, capacity, volume, interface, GPU, and power facts without discovery or benchmarking. |
| `backend/app/contexts/production/runtime/runtime_capability.py` | General Runtime capability contract. | ED-0050 | Defines approved capability kinds and supported, unsupported, degraded, or unknown status with first-class limitations. |
| `backend/app/contexts/production/runtime/runtime_source_capability.py` | Runtime source capability contract. | ED-0050 | Declares source schemes, host/volume scope, read access, adapters, and recorder compatibility without accessing sources. |
| `backend/app/contexts/production/runtime/runtime_observation_capability.py` | Runtime resource-observation capability contract. | ED-0050 | Maps supplied adapter modes to ED-0049 snapshot, finalization, write, read, and presence facts without collecting them. |
| `backend/app/contexts/production/runtime/runtime_readiness_capability.py` | Runtime readiness capability contract. | ED-0050 | Declares finalization methods, stability prerequisites, policy compatibility, and limitations without evaluating readiness. |
| `backend/app/contexts/production/runtime/runtime_capability_set.py` | Runtime capability-set contract. | ED-0050 | Deterministically normalizes general and specialized capabilities while preserving conflicting duplicate diagnostics. |
| `backend/app/contexts/production/runtime/runtime_resource_budget.py` | Runtime resource budget contract. | ED-0050 | Declares optional non-negative CPU, memory, I/O, network, concurrency, frequency, and burst limits without enforcement. |
| `backend/app/contexts/production/runtime/runtime_resource_policy.py` | Runtime resource-priority contract. | ED-0050 | Declares production-subordinate priority, GPU/optional-work rules, pressure responses, recovery, and limitations without measurement. |
| `backend/app/contexts/production/runtime/runtime_event_mode.py` | Runtime event-mode contract. | ED-0050 | Declares event, development, maintenance, or disabled mode; offline/network posture; retention; override; and event window. |
| `backend/app/contexts/production/runtime/runtime_collection_target.py` | Runtime collection-target contract. | ED-0050 | Retains opaque credential-safe source references, explicit context, and selected resource fact types without collection. |
| `backend/app/contexts/production/runtime/runtime_collection_plan.py` | Runtime collection-plan contract. | ED-0050 | Relates targets, observation capabilities, modes, readiness, resource, and event declarations without scheduling or execution. |
| `backend/app/contexts/production/runtime/runtime_readiness_policy_selection.py` | Runtime readiness-route selection. | ED-0050 | Embeds explicit ED-0049 parameters, policy identity/version, capability requirements, route, fallback, and limitations without policy invocation. |
| `backend/app/contexts/production/runtime/runtime_asset_assembly_plan.py` | Runtime asset-assembly declaration. | ED-0050 | Maps explicit future inputs to ED-0048 manifest vocabulary and privacy policy without constructing an asset or manifest. |
| `backend/app/contexts/production/runtime/runtime_limitation.py` | Runtime limitation contract. | ED-0050 | Retains first-class severity, affected capability/plan IDs, description, and caller-supplied introduction time. |
| `backend/app/contexts/production/runtime/runtime_health.py` | Runtime health contracts. | ED-0050 | Declares health, configuration validity, component availability, and reporting expectations without active checks. |
| `backend/app/contexts/production/runtime/runtime_availability.py` | Runtime availability contract. | ED-0050 | Declares available, limited, unavailable, disabled, or unknown work availability separately from health. |
| `backend/app/contexts/production/runtime/runtime_configuration.py` | Runtime configuration contract. | ED-0050 | Versioned immutable configuration graph with caller-supplied time and no persistence or remote mutation. |
| `backend/app/contexts/production/runtime/runtime_validation.py` | Runtime combination validator. | ED-0050 | Pure deterministic precedence-based validation of identity, schemas, capabilities, event constraints, plans, routes, assembly, health, availability, and limitations. |
| `backend/app/contexts/production/runtime/runtime_summary.py` | Runtime summary contract. | ED-0050 | Deterministic privacy-safe diagnostic projection without source locations, credentials, or metadata. |
| `backend/app/contexts/production/runtime/runtime_contract_validation.py` | Runtime contract validation helpers. | ED-0050 | Central timezone, numeric, normalization, deep metadata immutability, and credential-safety helpers with no I/O. |
| `backend/app/contexts/production/software_agent_runtime/` | Software Agent Runtime lifecycle package. | ED-0051 | Exactly one synchronous process-local Agent lifecycle over ED-0050 contracts, with no service, background work, media, filesystem, recorder, readiness, asset, transfer, queue, persistence, or network behavior. |
| `backend/app/contexts/production/software_agent_runtime/README.md` | Software Agent Runtime package guide. | ED-0051 | Documents explicit startup, validation authority, lifecycle graph, pressure/resume, permission, shutdown, idempotency, synchronization, notification failure, health/availability, production priority, and strict boundaries. |
| `backend/app/contexts/production/software_agent_runtime/__init__.py` | Software Agent Runtime package exports. | ED-0051 | Exports the concrete Agent lifecycle, immutable requests/results/state, reason/outcome vocabularies, dependency bundle, and three notification ports. |
| `backend/app/contexts/production/software_agent_runtime/software_agent_runtime.py` | Concrete Software Agent lifecycle. | ED-0051 | Owns explicit lifecycle operations, one-lock immutable copy-and-swap state, revision protection, idempotency, pressure response, deterministic shutdown, and post-commit notifications. |
| `backend/app/contexts/production/software_agent_runtime/agent_runtime_lifecycle.py` | Agent lifecycle vocabularies. | ED-0051 | Defines exact states, execution permissions, operations, outcomes, reasons, and deterministic reason ordering. |
| `backend/app/contexts/production/software_agent_runtime/agent_runtime_requests.py` | Agent lifecycle operation contracts. | ED-0051 | Defines immutable prepare, start, pressure, resume, cancellation, stop, and failure inputs with explicit aware timestamps. |
| `backend/app/contexts/production/software_agent_runtime/agent_runtime_snapshot.py` | Agent lifecycle output contracts. | ED-0051 | Defines immutable snapshots, transitions, operation results, notification failures, and privacy-safe summaries. |
| `backend/app/contexts/production/software_agent_runtime/agent_runtime_dependencies.py` | Agent dependency bundle. | ED-0051 | Retains only the explicit lifecycle, health, and availability notification sinks. |
| `backend/app/contexts/production/software_agent_runtime/agent_runtime_derivation.py` | Lifecycle health and availability derivation. | ED-0051 | Deterministically derives ED-0050-compatible declarations and IDs from lifecycle state and caller-supplied time. |
| `backend/app/contexts/production/software_agent_runtime/agent_runtime_contract_validation.py` | Agent contract validation helpers. | ED-0051 | Provides timezone, normalization, deep immutability, credential safety, and deterministic operation fingerprint helpers without I/O. |
| `backend/app/contexts/production/software_agent_runtime/ports/` | Agent notification ports. | ED-0051 | Defines lifecycle, health, and availability publication protocols without persistence, retries, monitoring, or Production Events. |
| `backend/app/contexts/production/software_agent_runtime/ports/__init__.py` | Agent notification port exports. | ED-0051 | Exports the three narrow synchronous notification protocols. |
| `backend/app/contexts/production/software_agent_runtime/ports/lifecycle_event_sink.py` | Agent lifecycle notification protocol. | ED-0051 | Receives immutable lifecycle transitions without persistence, retry, or Production Event creation. |
| `backend/app/contexts/production/software_agent_runtime/ports/runtime_health_sink.py` | Agent health notification protocol. | ED-0051 | Receives immutable ED-0050-compatible health declarations without active checks. |
| `backend/app/contexts/production/software_agent_runtime/ports/runtime_availability_sink.py` | Agent availability notification protocol. | ED-0051 | Receives immutable ED-0050-compatible availability declarations without liveness monitoring. |
| `backend/app/contexts/production/media_collection/` | Media candidate collection package. | ED-0052 | One explicit synchronous bounded coordinator, immutable process-local state, discovery/observation ports, typed queries, replay, revision, conflict, and accumulation semantics without concrete media access or downstream execution. |
| `backend/app/contexts/production/media_collection/README.md` | Media collection package guide. | ED-0052 | Documents authority, permission checkpoints, bounds, ordering, deduplication, conflicts, accumulation, lock boundary, partial facts, replay, deployment neutrality, and strict exclusions. |
| `backend/app/contexts/production/media_collection/__init__.py` | Media collection package exports. | ED-0052 | Exports the coordinator, immutable requests/results/records/summaries, enums, dependency bundle, and seven narrow ports. |
| `backend/app/contexts/production/media_collection/media_candidate_collection_coordinator.py` | Concrete media candidate collection coordinator. | ED-0052 / ED-0064 | Composes internal validation and port collaborators while retaining explicit cycle orchestration, candidate/observation merging, conflicts, atomic process-local commits, idempotency, history, and typed queries. |
| `backend/app/contexts/production/media_collection/_media_collection_state.py` | Internal coordinator state values and deterministic identity helpers. | ED-0064 | Isolates immutable coordinator state, per-cycle working facts, stable observation ordering, and deterministic IDs without changing public contracts. |
| `backend/app/contexts/production/media_collection/_media_collection_plan_validator.py` | Internal runtime-plan and permission validator. | ED-0064 | Owns pre-cycle Runtime/configuration/port validation and repeated Agent permission checkpoints while preserving ED-0052 outcomes. |
| `backend/app/contexts/production/media_collection/_media_collection_ports.py` | Internal collection-port gateway. | ED-0064 | Owns discovery/observation port invocation, exception visibility, and returned-contract normalization while preserving bounded no-retry behavior. |
| `backend/app/contexts/production/media_collection/media_collection_contracts.py` | Media collection contracts. | ED-0052 / ED-0053 | Defines immutable discovery/observation/cycle inputs and results, candidate records, conflicts, snapshots, summaries, typed queries, and source-location-first canonical discovery ordering needed by deterministic bounded adapters. |
| `backend/app/contexts/production/media_collection/media_collection_lifecycle.py` | Media collection vocabularies. | ED-0052 | Defines exact discovery, observation, candidate-status, cycle, query, conflict, and deterministic reason values. |
| `backend/app/contexts/production/media_collection/media_collection_dependencies.py` | Media collection dependency bundle. | ED-0052 | Retains only the read-only Agent state, discovery, and five ED-0049 observation ports. |
| `backend/app/contexts/production/media_collection/media_collection_validation.py` | Media collection validation helpers. | ED-0052 | Provides deterministic fingerprinting, normalization, deep metadata freezing, time, and bound validation without I/O. |
| `backend/app/contexts/production/media_collection/ports/` | Media collection port package. | ED-0052 | Defines one read-only Agent state port, one discovery port, and five objective observation collection ports. |
| `backend/app/contexts/production/media_collection/ports/__init__.py` | Media collection port exports. | ED-0052 | Exports all seven narrow synchronous interfaces. |
| `backend/app/contexts/production/media_collection/ports/agent_execution_state_port.py` | Agent execution-state port. | ED-0052 | Reads one immutable ED-0051 snapshot without lifecycle mutation, callbacks, waiting, or monitoring. |
| `backend/app/contexts/production/media_collection/ports/media_candidate_discovery_port.py` | Media candidate discovery port. | ED-0052 | Abstract one-shot candidate-fact discovery with no concrete source access. |
| `backend/app/contexts/production/media_collection/ports/observation_collection_ports.py` | Media observation collection ports. | ED-0052 | Abstract snapshot, finalization, write, read, and presence fact suppliers without readiness evaluation. |
| `backend/app/contexts/production/local_filesystem_discovery/` | Bounded local-filesystem candidate discovery package. | ED-0053 | Exactly one synchronous stateless ED-0052 discovery-port implementation plus immutable configuration, target binding, eligibility, identity, reason, limitation, and validation contracts. |
| `backend/app/contexts/production/local_filesystem_discovery/README.md` | Local-filesystem discovery guide. | ED-0053 | Documents single-file and shallow-directory scope, explicit bounds and eligibility, symlink and content protections, deterministic identity/time, and strict separation from readiness and assets. |
| `backend/app/contexts/production/local_filesystem_discovery/__init__.py` | Local-filesystem discovery exports. | ED-0053 | Exports the sole concrete adapter and immutable ED-0053 configuration, binding, policy, identity, scope, reason, and limitation contracts. |
| `backend/app/contexts/production/local_filesystem_discovery/local_filesystem_candidate_discovery_adapter.py` | Concrete bounded local-filesystem discovery adapter. | ED-0053 | Validates one ED-0052 request against ED-0050 configuration, inspects one exact scope with `lstat` and bounded `scandir`, and returns canonical deterministic ED-0049 candidates without content reads or retained state. |
| `backend/app/contexts/production/local_filesystem_discovery/local_filesystem_discovery_contracts.py` | Local-filesystem discovery contracts. | ED-0053 | Defines immutable adapter configuration, target binding, explicit eligibility, scope, symlink, extension, and identity-strength contracts. |
| `backend/app/contexts/production/local_filesystem_discovery/local_filesystem_discovery_reason.py` | Local-filesystem discovery reason and limitation vocabularies. | ED-0053 | Defines deterministic typed request, filesystem, filtering, bound, identity, partial, and semantic-boundary diagnostics. |
| `backend/app/contexts/production/local_filesystem_discovery/local_filesystem_validation.py` | Local-filesystem discovery validation helpers. | ED-0053 | Provides credential-safe absolute lexical path validation, wildcard/parent-traversal rejection, extension normalization, immutable hint mappings, metadata freezing, and aware-time reuse without filesystem access. |
...
| `backend/tests/README.md` | Backend test suite guide. | ED-0002 / ED-0041 / ED-0047 through ED-0053 | Documents implemented backend coverage through ED-0053, including Runtime, Agent lifecycle, bounded media collection, local-filesystem scope, identity, security, safety, and architecture boundaries. |
...
| `backend/tests/test_completed_media_asset_deployment_neutrality.py` | Completed media deployment-neutrality tests. | ED-0048 / ED-0053 correction | Covers Agent/Node/external/Development/unknown peer treatment, first-class provenance without metadata, capability independence, naming independence, asset meaning, identity semantics, and deterministic normalization. |
...
| `backend/tests/asset_readiness_fixtures.py` | Asset readiness test fixture factory. | ED-0049 | Creates deterministic synthetic candidate, request, parameter, snapshot, finalization, write, read, presence, bundle, and policy inputs with no real media or I/O. |
| `backend/tests/test_asset_readiness_contracts.py` | Asset readiness contract tests. | ED-0049 | Covers candidate/resource separation, observation vocabularies and validation, bundle normalization, explicit parameters, requests, reasons, summaries, privacy, immutability, and ED-0048 completion limitations. |
| `backend/tests/test_asset_readiness_policy.py` | Conservative asset readiness policy tests. | ED-0049 | Covers strong, stable, guarded-manual, insufficient, unsupported, invalid, and blocking routes; latest-state semantics; timestamps; declarations; limitations; and all reference scenarios. |
| `backend/tests/test_asset_readiness_stability.py` | Asset stability derivation tests. | ED-0049 | Covers two/three snapshots, elapsed thresholds, changed facts, intermediate contradictions, deterministic ordering and latest-window selection, limitations, validation, and immutability. |
| `backend/tests/test_asset_readiness_conflicts.py` | Asset readiness conflict tests. | ED-0049 | Covers conflicting duplicate IDs, resource token/host/volume changes, replacement, simultaneous incompatible states, post-finalization growth/modification, latest presence, reason ordering, and Runtime mismatch. |
| `backend/tests/test_asset_readiness_deployment_neutrality.py` | Asset readiness deployment-neutrality tests. | ED-0049 / ED-0053 correction | Covers Agent, Node, external, Development, and unknown profile parity; first-class candidate provenance; profile-independent parameters and results; naming independence; and optional capability routes. |
| `backend/tests/test_asset_readiness_architecture.py` | Asset readiness architecture tests. | ED-0049 | Verifies exact package and policy scope, serialization readiness, documentation and registration, ED-0048 reuse, and absence of monitoring, I/O, Runtime, transfer, downstream state, APIs, workers, AI, or frontend behavior. |
| `backend/tests/runtime_fixtures.py` | Runtime test fixture factory. | ED-0050 | Creates deterministic synthetic Runtime graphs, explicit ED-0049 parameters, distinct timestamps, and configurable capability/route/resource/profile scenarios without I/O. |
| `backend/tests/test_runtime_contracts.py` | Runtime contract tests. | ED-0050 | Covers the canonical graph, identity, versions, host, configuration, assembly declarations, summaries, timestamps, metadata safety, immutability, and excluded active objects. |
| `backend/tests/test_runtime_capabilities.py` | Runtime capability tests. | ED-0050 | Covers approved vocabularies, general/specialized mappings, deterministic normalization, conflicts, source scope, readiness declarations, and explicit unsupported/degraded states. |
| `backend/tests/test_runtime_resource_policy.py` | Runtime resource and event-policy tests. | ED-0050 | Covers budgets, production-subordinate priority, pressure responses, GPU/optional-work constraints, offline network posture, source retention, disabled mode, and declarative-only behavior. |
| `backend/tests/test_runtime_collection_plan.py` | Runtime collection-plan tests. | ED-0050 | Covers credential-safe targets, policy references, source host/volume/scheme compatibility, observation availability/type/mode selection, ordering, boundaries, and no collection execution. |
| `backend/tests/test_runtime_readiness_selection.py` | Runtime readiness-selection tests. | ED-0050 | Covers explicit ED-0049 parameters, strong/stability routes, stable identity, read/write/presence/finalization requirements, fallback, policy support, ED-0048 assembly mapping, and non-authoritative hints. |
| `backend/tests/test_runtime_health.py` | Runtime health and availability tests. | ED-0050 | Covers healthy/degraded/unhealthy/unknown declarations, available/limited/unavailable/disabled/unknown status, limitation references, event compatibility, deterministic summaries, and no active checks. |
| `backend/tests/test_runtime_deployment_neutrality.py` | Runtime deployment-neutrality tests. | ED-0050 | Covers Agent/Node/external/development/unknown peers, combination semantics, no profile exemption or trust ranking, distinct Runtime isolation, unknown and invalid precedence, and no process lifecycle. |
| `backend/tests/test_runtime_architecture.py` | Runtime architecture tests. | ED-0050 | Verifies exact contract scope, one Runtime graph, immutable serialization-ready shapes, documentation and registration, ED-0048/ED-0049 reuse, and absence of services, monitoring, I/O, execution, transfer, downstream state, APIs, workers, AI, or frontend behavior. |
| `backend/tests/software_agent_runtime_fixtures.py` | Software Agent Runtime test fixtures. | ED-0051 | Creates deterministic Runtime instances, operation requests, pressure declarations, and recording/failing notification sinks without real work or I/O. |
| `backend/tests/test_software_agent_runtime_contracts.py` | Agent lifecycle contract tests. | ED-0051 | Covers exact vocabularies, immutable inputs/outputs, construction, lineage, timestamps, metadata safety, summaries, and excluded payloads. |
| `backend/tests/test_software_agent_runtime_startup.py` | Agent startup tests. | ED-0051 | Covers explicit prepare/start, ED-0050 validation and drift, profile policy, disabled modes, dependencies, identity, configuration authority, pressure, and isolation. |
| `backend/tests/test_software_agent_runtime_lifecycle.py` | Agent lifecycle behavior tests. | ED-0051 | Covers permitted and forbidden transitions, history, revision consistency, failures, summaries, idempotency, conflict, and stale protection. |
| `backend/tests/test_software_agent_runtime_pressure.py` | Agent pressure and resume tests. | ED-0051 | Covers all pressure categories, conservative suspension, no automatic resume, explicit resume, derived permission/health/availability, replay, stale, and invalid inputs. |
| `backend/tests/test_software_agent_runtime_shutdown.py` | Agent cancellation and stop tests. | ED-0051 | Covers deterministic stop from every approved state, terminal behavior, cancellation identity/idempotency, permission revocation, and no process control. |
| `backend/tests/test_software_agent_runtime_concurrency.py` | Agent concurrency tests. | ED-0051 | Uses barriers and controlled events for simultaneous start/pressure, start versus stop, pressure versus resume/cancel, and duplicate-stop races. |
| `backend/tests/test_software_agent_runtime_failure_atomicity.py` | Agent atomicity and notification tests. | ED-0051 | Covers observable rejection invariance, deterministic publication order, sink failure reporting, committed-state preservation, and no retry. |
| `backend/tests/test_software_agent_runtime_deployment_neutrality.py` | Agent deployment-neutrality tests. | ED-0051 | Covers unchanged ED-0050 authority, Node adapter rejection without semantic distrust, reusable lifecycle contracts, and absence of media-specific behavior. |
| `backend/tests/test_software_agent_runtime_architecture.py` | Agent architecture boundary tests. | ED-0051 | Verifies one concrete lifecycle, exact source scope, documentation/registration, and absence of background, I/O, monitoring, media, downstream state, service, API, worker, AI, or frontend behavior. |
| `backend/tests/media_collection_fixtures.py` | Media collection test fixtures. | ED-0052 | Creates deterministic Agent snapshots, candidates, objective observations, requests, and recording/failing ports without real media or I/O. |
| `backend/tests/test_media_collection_contracts_and_cycle.py` | Media collection contract and cycle tests. | ED-0052 | Covers exact vocabularies, validation, inert construction, normal ordering, immutable results, bundles, history, and typed absence. |
| `backend/tests/test_media_collection_permission_replay_and_concurrency.py` | Media collection permission, replay, and concurrency tests. | ED-0052 | Covers normal/reduced/essential permission, interruption, exact/conflicting replay, stale revision, active-cycle exclusion, slow ports, and Development placement. |
| `backend/tests/test_media_collection_dedup_budgets_and_failures.py` | Media collection identity, budget, and failure tests. | ED-0052 | Covers rediscovery, candidate/asset/resource/observation conflicts, accumulation, port exception isolation, explicit bounds, and deterministic queries. |
| `backend/tests/test_media_collection_validation_and_architecture.py` | Media collection validation and architecture tests. | ED-0052 | Covers configuration drift, Agent identity, dependencies, profile placement, one coordinator, lock-safe scope, and forbidden infrastructure/downstream boundaries. |
| `backend/tests/local_filesystem_discovery_fixtures.py` | Local-filesystem discovery fixtures. | ED-0053 | Builds synthetic Runtime, target binding, eligibility, adapter, and request graphs over pytest temporary paths with no background or production infrastructure. |
| `backend/tests/test_local_filesystem_discovery_contracts.py` | Local-filesystem discovery contract tests. | ED-0053 | Covers immutable configuration, explicit policy, target binding, timestamps, metadata, credentials, path validation, unique mapping, and inert construction. |
| `backend/tests/test_local_filesystem_discovery_adapter.py` | Local-filesystem discovery adapter tests. | ED-0053 | Covers single-file and shallow-directory results, eligibility, context/hints, local/mounted/unsupported schemes, missing/mismatch outcomes, read-only behavior, active/zero-byte candidates, timestamps, replay, and path substitution. |
...
| `docs/adr/ADR-0028-devcon-external-integration-boundary.md` | Bounded Devcon external integration decision. | ED-0065 | Retroactively records the shipped program-read and guarded human-confirmed enrichment-write boundary without authorizing a new external write. |
| `docs/security/` | Security, dependency, and software-composition evidence. | ED-0066 | Contains the current dependency-license assessment and generated SBOM artifacts; not legal advice or a readiness claim. |
| `docs/security/dependency-license-sbom-2026-08-21.md` | Backend/frontend license and SBOM assessment. | ED-0066 | Records copyleft findings, native-wheel uncertainty, npm lock-graph warnings, and decision options without changing dependencies. |
| `docs/security/sbom/backend.cdx.json` | Backend CycloneDX 1.5 SBOM. | ED-0066 | Lockfile-backed inventory across default, development, and transcription groups. |
| `docs/security/sbom/backend-licenses.json` | Backend installed-package license inventory. | ED-0066 | Supplements uv's SBOM with distribution metadata license declarations for the Windows resolution. |
| `docs/security/sbom/frontend.cdx.json` | Frontend CycloneDX 1.6 SBOM. | ED-0066 | Lockfile-backed inventory generated with npm graph warnings retained in the assessment. |
