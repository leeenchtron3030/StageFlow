...
| [ADR-0032](ADR-0032-render-durable-operation.md) | Render Durable Operation by additive generalization of the ADR-0025 substrate; immutable Rendered Output identity; video-only first render profile; configured render output root | Accepted | Accepted by the owner 2026-09-25; implemented as ED-0087; amendment 3 (profile v2, constant frame rate) 2026-09-26 for ED-0089 |
| [ADR-0033](ADR-0033-production-media-timing-inspection.md) | Production media timing inspection: a `media_timing` Durable Operation using an operator-installed LGPL `ffprobe` by explicit path that records Observed `creation_time`/durations and a Derived `creation_time_plus_duration` v1 interval as advisory, `unqualified` MTE; consumers stay advisory | Proposed | Drafted 2026-09-26 at the owner's direction ("media timing first"); awaiting acceptance; implementation planned as ED-0090 |
